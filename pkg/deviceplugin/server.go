...
func (srv *server) setupAndServe(namespace string, devicePluginPath string, kubeletSocket string) error {
	...
	for srv.getState() == serving {
		...
		go func() {
			klog.V(1).Infof("Start server for %s at: %s\n", srv.devType, pluginSocket)
		...
		klog.V(1).Infof("Device plugin for %s registered\n", srv.devType)
		...
		if srv.getState() == serving {
			srv.grpcServer.Stop()
			klog.V(1).Infof("Socket %s removed, restarting\n", pluginSocket)
		} else {
			klog.V(1).Infof("Socket %s shut down\n", pluginSocket)
		}
	}
