...
func (dp *DevicePlugin) getDpdkDevice(id string) (string, error) {
	...
	case "vfio-pci":
		...
		s := path.Base(group)
		klog.V(1).Infof("The vfio device group detected is %v\n", s)
		return s, nil
	}
...
func (dp *DevicePlugin) getDpdkDeviceSpecs(id string) ([]pluginapi.DeviceSpec, error) {
	...
	klog.V(1).Infof("%s device: corresponding DPDK device detected is %s\n", id, dpdkDeviceName)
...
func (dp *DevicePlugin) scan() (dpapi.DeviceTree, error) {
	...
	for _, driver := range append([]string{dp.dpdkDriver}, dp.kernelVfDrivers...) {
		...
		if err != nil {
			klog.Warningf("Can't read sysfs for driver as Driver %s is not available: Skipping\n", driver)
			continue
		}
